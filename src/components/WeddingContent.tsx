...
export default function WeddingContent({ config }: { config: WeddingConfig }) {
	...
	const guestbookVisible = config.showGuestbook !== false && !!config.guestbookScriptUrl
	const hiddenSections = [
		...(config.showAccount === false ? ['account'] : []),
		...(guestbookVisible ? [] : ['guestbook']),
	]
	...
	return (
		<>
			...
			<div className="hidden lg:flex min-h-screen w-full">
				...
				{/* 우측: 정보 영역 스크롤 2/3 */}
				<div id="scroll-content" className="w-2/3 bg-cream overflow-y-auto h-screen">
					<div className="relative">
						<ScrollIndicator
							scrollContainerId="scroll-content"
							labels={config.labels.nav}
